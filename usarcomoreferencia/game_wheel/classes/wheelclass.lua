...
-- Funkcja do konwersji vocation na ID dla Wheel of Destiny
...
function WheelOfDestiny.onMouseMove(widget, position, offset)
  ...
  if WheelOfDestiny.pointInvested[index] > 0 then
    wheelOfDestinyWindow.info.tabContent.information.tabContent.dedication2:setColor("#dfdfdf")
  else
    wheelOfDestinyWindow.info.tabContent.information.tabContent.dedication2:setColor("#808080")
  end
  ...
  if type(conviction) == "string" then
    ...
    if WheelOfDestiny.pointInvested[index] >= bonus.maxPoints then
      wheelOfDestinyWindow.info.tabContent.information.tabContent.conviction2:setColor("#dfdfdf")
    else
      wheelOfDestinyWindow.info.tabContent.information.tabContent.conviction2:setColor("#808080")
    end
    ...
    if points >= bonus.maxPoints then
      ...
      if bonus and table.contains(VesselIndex[bonus.domain - 1], index - 1) then
        ...
        if gem then
          ...
          if #enabled == 0 and gem.lesserBonus and gem.lesserBonus > -1 then
          ...
          elseif #enabled == 1 and gem.regularBonus and gem.regularBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
            ...
            table.insert(WheelOfDestiny.vesselEnabled[bonus.domain - 1], index)
          elseif #enabled == 2 and gem.supremeBonus and gem.supremeBonus > -1 then
...
function WheelOfDestiny.checkFilledVessels(originalIndex)
	...
	for _, id in pairs(WheelOfDestiny.vesselEnabled[bonus.domain - 1]) do
		...
		if lastModInserted == 0 and gem.lesserBonus and gem.lesserBonus > -1 then
			widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
			widget:setImageClip(30 * gem.lesserBonus .. " 0 30 30")
			modIcon:setVisible(true)
			WheelOfDestiny.equipedGemBonuses[id] = {bonusID = gem.lesserBonus, supreme = false, gemID = gem.gemID}
			lastModInserted = 1
		elseif lastModInserted == 1 and gem.regularBonus and gem.regularBonus > -1 then
			widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
			widget:setImageClip(30 * gem.regularBonus .. " 0 30 30")
			modIcon:setVisible(true)
			WheelOfDestiny.equipedGemBonuses[id] = {bonusID = gem.regularBonus, supreme = false, gemID = gem.gemID}
			lastModInserted = 2
		elseif lastModInserted == 2 and gem.supremeBonus and gem.supremeBonus > -1 then
			widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-suprememods")
			widget:setImageClip(35 * gem.supremeBonus .. " 0 35 35")
			widget:setSize(tosize("35 35"))
			modIcon:setVisible(true)
...
function WheelOfDestiny.onDestinyWheel(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  if not table.isIn({1, 2, 3, 4, 5}, vocationId) then
    -- g_client.setInputLockWidget(nil) -- Function not available
  ...
  if not wheelWindow:isVisible() then
    wheelWindow:show()
    WheelOfDestiny.resetPassiveFocus()
    -- g_client.setInputLockWidget(wheelWindow) -- Function not available
  end
  ...
  local bankMoney = player:getResourceBalance(ResourceTypes.BANK_BALANCE) or 0
  local characterMoney = player:getResourceBalance(ResourceTypes.GOLD_EQUIPPED) or 0
  local lesserFragment = player:getResourceBalance(ResourceTypes.LESSER_FRAGMENT) or 0
  local greaterFragment = player:getResourceBalance(ResourceTypes.GREATER_FRAGMENT) or 0
    ...
    if widget then
      ...
      if bonus and table.contains(VesselIndex[bonus.domain - 1], id - 1) then
        ...
        if gem and pointInvested >= bonus.maxPoints then
          if bonus.modType == 0 and gem.lesserBonus and gem.lesserBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
            widget:setImageClip(30 * gem.lesserBonus .. " 0 30 30")
            modIcon:setVisible(true)
          elseif bonus.modType == 1 and gem.regularBonus and gem.regularBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
            widget:setImageClip(30 * gem.regularBonus .. " 0 30 30")
            modIcon:setVisible(true)
          elseif bonus.modType == 2 and gem.supremeBonus and gem.supremeBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-suprememods")
            widget:setImageClip(35 * gem.supremeBonus .. " 0 35 35")
            widget:setSize(tosize("35 35"))
            modIcon:setVisible(true)
...
function onAddOne(index)
  ...
  -- Blokada: sprawdź, czy są dostępne punkty
  local totalPoints = WheelOfDestiny.points + (WheelOfDestiny.extraGemPoints + WheelOfDestiny.scrollPoints)
  if WheelOfDestiny.usedPoints >= totalPoints then
    return -- nie pozwól na wydanie więcej niż się ma
  end
...
function WheelOfDestiny.configureDedication(index)
  ...
  if WheelOfDestiny.pointInvested[index] > 0 then
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor("#dfdfdf")
  else
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor("#808080")
  end
end
...
function WheelOfDestiny.configureConviction(index)
  ...
  if type(conviction) == "string" then
    ...
    if WheelOfDestiny.pointInvested[index] >= bonus.maxPoints then
      wheelOfDestinyWindow.selection.tabContent.conviction:setColor("#dfdfdf")
    else
      wheelOfDestinyWindow.selection.tabContent.conviction:setColor("#808080")
    end
  elseif type(conviction) == "table" then
    wheelOfDestinyWindow.selection.tabContent.conviction:setTooltip(tooltip)
...
function WheelOfDestiny.create(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  ...
  if WheelOfDestiny.vocationId == 0 then
    ...
    if player then
      ...
      local wheelVocation = translateWheelVocation(serverVocation)
      WheelOfDestiny.vocationId = wheelVocation
    end
  end 
...
function WheelOfDestiny.configurePassives()
  ...
  for i = 0, 3 do
    ...
    if data and (data.supremeBonus or 0) > 0 and filledCount == 3 then
      local vocationId = translateVocation(WheelOfDestiny.vocationId)
      local supremeList = (data.supremeBonus or 0) > 5 and VocationSupremeMods[vocationId] or FlatSupremeMods
      if supremeList then
        local bonus = supremeList[data.supremeBonus or 0]
        if bonus and bonus.domain then
          local gemValue = getBonusValueUpgrade(data.supremeBonus or 0, data.gemID or 0, true, true)
...
function WheelOfDestiny.onPreparePresetClick(list, selection, oldSelection)
  ...
  -- g_client.setInputLockWidget(checkSavePresetWindow) -- Function not available
end
