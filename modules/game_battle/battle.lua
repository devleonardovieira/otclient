...
-- Evita múltiplas inicializações e permite recriação segura da janela
local battleInitialized = false

local ageNumber = 1
local ages = {}

-- Retorna um parent seguro para MiniWindows: prioriza painel direito se visível,
-- senão cai para o RootPanel como o minimap faz
local function getSafeMiniWindowParent()
  local right = modules.game_interface.getRightPanel()
  if right and right.isExplicitlyVisible and right:isExplicitlyVisible() then
    -- se for um container e estiver com largura > 0, usamos
    if right.getWidth and right:getWidth() > 0 then
      return right
    end
  end
...
function init()
  if battleInitialized and battleWindow and not battleWindow:isDestroyed() then
    return
  end
...
--[[ 	battleButton = modules.client_topmenu.addRightButton("battleButton", tr("Battle") .. " (Ctrl+B)", "/images/topbuttons/icon_battle", toggle, false, 6)
    ...
    battleWindow = g_ui.loadUI("battle", getSafeMiniWindowParent())
...
function terminate()
    ...
    if mouseWidget and not mouseWidget:isDestroyed() then
      mouseWidget:destroy()
    end
...
function toggle()
    -- Recria a janela se ainda não existir ou foi destruída
    if not battleWindow or battleWindow:isDestroyed() then
      init()
    end

    -- Garanta que o parent seja seguro (RightPanel se visível, senão RootPanel)
    local safeParent = getSafeMiniWindowParent()
    if safeParent and battleWindow:getParent() ~= safeParent then
      battleWindow:setParent(safeParent)
    end

    if battleWindow:isExplicitlyVisible() then
      battleWindow:close()
    else
      -- Abra e maximize para garantir visibilidade
      battleWindow:open()
      battleWindow:maximize()
      battleWindow:raise()
    end
end
...
function checkCreatures()
	...
	if g_platform.isMobile() and #creatures > 0 then
